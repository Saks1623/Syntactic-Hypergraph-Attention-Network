...
def clean_document_syntax(doc_sentence_list,dataset):
    '''Light cleaning for syntectic'''
    stemmer = WordNetLemmatizer()
    syntax_docs = []
    for doc_sentences in doc_sentence_list:
        doc_tokens = []
        for sent in doc_sentences:
           # use your simple string‐cleaner to strip quotes/backslashes
            s = clean_str_simple_version(sent, dataset=None)
            words = word_tokenize(s.lower())
            doc_tokens.append(words)
        syntax_docs.append(doc_tokens)
    return syntax_docs
...
class Data():
    def __init__(self, data, max_num_sentence, keywords_dic, syntactic_dic, num_categories, LDA = False,syn=False):
        inputs = data[0]
        self.inputs = np.asarray(inputs,dtype=object) 
        self.targets = np.asarray(data[1])
        self.length = len(inputs)
        self.keywords = keywords_dic
        self.syntactic= syntactic_dic
        self.syn=syn
        self.LDA = LDA
    ...
    def generate_batch(self, batch_size, shuffle = False):
        ...
        slices = np.array_split(indices, n_batch)
        return slices
    ...
    def get_slice(self, iList):
        ...
        if self.LDA:
            num_edge = [e + self.num_categories for e in num_edge]
        if self.syn:
            syn_counts = [len(self.syntactic[idx]) for idx in iList]
            num_edge = [e + sc for e, sc in zip(num_edge, syn_counts)]
            ...
            if self.LDA:
                for w in node:
                    if w in self.keywords:
                        temp = self.keywords[w]
                                                
                        rows += [node_dic[idx][w]]*len(temp)
                        cols += [cur_offset + t for t in temp]
                        vals += [1.0]*len(temp)
                cur_offset += self.num_categories
            
            #syntactic edges
            if self.syn:
                syn_dict=self.syntactic[iList[idx]]
                for edge_id,members in syn_dict.items():
                    for word_id in members:
                        if word_id in node_dic[idx]:
                            rows.append(node_dic[idx][word_id])
                            cols.append(cur_offset+edge_id)
                            vals.append(1.0)

            u_H = sp.coo_matrix((vals, (rows, cols)), shape=(max_n_node, max_n_edge))
            HT.append(torch.FloatTensor(u_H.T.todense()))
